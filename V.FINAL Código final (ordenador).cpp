...
void modificar_bebida(lista* bebida) {
	...
	if (p == NULL) {
		...
		do {
			p->bebida_lista.proporcion.A = bebida_personalizada('A');
			p->bebida_lista.proporcion.B = bebida_personalizada('B');
			p->bebida_lista.proporcion.C = bebida_personalizada('C');
			scanf_s("%c", &c);
			if (p->bebida_lista.proporcion.A + p->bebida_lista.proporcion.B + p->bebida_lista.proporcion.C != 100) {
				printf("Los porcentajes introducidos no suman 100, vuelva a introducirlos\n");
			}
		} while (p->bebida_lista.proporcion.A + p->bebida_lista.proporcion.B + p->bebida_lista.proporcion.C != 100);
	}
	return;
}
